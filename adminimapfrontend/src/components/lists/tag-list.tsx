...
interface IProps {
  tagArray: Array<ITag>;
  handleTagCloseClick: (tag: number) => void;
}

const TagList: React.FunctionComponent<IProps> = (props) => {
  return (
    <div className="w4-flex-wrap">
      {props.tagArray.map(tag => (
        <div key={tag.number} className="w4-tag w4-theme w4-flex">
          ...
          <div className="w4-button w4-button-tag-close" onClick={() => props.handleTagCloseClick(tag.number)}>
